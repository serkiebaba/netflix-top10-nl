...
def parse_titles_from_table(tbl):
    ...
    for tr in tbl.find_all("tr"):
        ...
        # Titel: prefer <td class*=title>, anders 1e <a>, anders langste tekst buiten 1e kolom
        title = ""
        title_cell = None
        for td in cells[1:]:
        ...
        if title_cell:
            ...
            if a and a.get_text(strip=True):
                ...
                # kies cel met meeste tekst (excl. kolom 0)
...
def build_metas(items, is_series=True):
    metas = []
    for rank, name in items:
        tmdb_id, poster_path = tmdb_lookup(name, is_series=is_series)
        typ = "series" if is_series else "movie"
        idv = (f"tmdb:tv:{tmdb_id}" if is_series else f"tmdb:movie:{tmdb_id}") if tmdb_id else f"flix-fallback-{'s' if is_series else 'm'}{rank}"
        metas.append({
            "id": idv,
            "type": typ,
            "name": name,
...
def main():
    ...
    if shows_list:
        with open(OUT_SERIES, "w", encoding="utf-8") as f:
            json.dump({"metas": build_metas(shows_list, is_series=True)}, f, ensure_ascii=False, indent=2)
    ...
    if movies_list:
        with open(OUT_MOVIES, "w", encoding="utf-8") as f:
            json.dump({"metas": build_metas(movies_list, is_series=False)}, f, ensure_ascii=False, indent=2)
